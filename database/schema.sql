...
VALUES (
        'Employee',
        'Basic user with access to training materials and surveys',
        '["view_courses", "take_surveys", "view_reports"]'
    ),
    (
        'Manager',
        'Team lead with additional management capabilities',
        '["view_courses", "take_surveys", "view_reports", "manage_team", "approve_requests"]'
    ),
    (
        'Admin',
        'Administrator with full system access except system configuration',
        '["all_permissions"]'
    ),
    (
        'SystemAdmin',
        'System administrator with complete access',
        '["all_permissions", "system_config", "user_management"]'
    );
...
INSERT INTO
    training_domains (domain_name, description)
VALUES (
        'IGL',
        'Industrial Gas and Liquids training domain'
    ),
    (
        'Admin',
    ...
    (
        'CNG_Projects_and_Steel',
        'CNG Projects and Steel operations training'
    ),
    (
        'CNG_OM',
        'CNG Operations and Maintenance training'
    );
...
CREATE TABLE training_needs (
    ...
    current_skill_level ENUM(
        'None',
        'Basic',
        'Intermediate',
        'Advanced'
    ) DEFAULT 'None',
    desired_skill_level ENUM(
        'Basic',
        'Intermediate',
        'Advanced',
        'Expert'
    ) DEFAULT 'Basic',
    preferred_delivery_method ENUM(
        'In-person',
        'Online',
        'Hybrid',
        'Self-study'
...
SELECT
    ...
    COUNT(
        CASE
            WHEN ce.completion_status = 'Completed' THEN 1
        END
    ) as completed_enrollments,
    COUNT(
        CASE
            WHEN ce.completion_status = 'In Progress' THEN 1
        END
    ) as in_progress_enrollments,
    -- ✅ This ensures every course shows a count, even if zero
    COUNT(DISTINCT
        CASE
            WHEN ce.completion_status = 'In Progress' THEN c.id
        END
    ) as course_with_in_progress_flag,
...
edit this code to count total courses with completion_status = 'In Progress' and include that count in the output.